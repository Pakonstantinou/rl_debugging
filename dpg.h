...
//    std::shared_ptr<criticgeneral> critic;
 ...
 public:
    ...
    dpg(std::shared_ptr<policygeneral> pol, std::shared_ptr<criticgeneral> cr, std::shared_ptr<environment> envi, double policy_learning_rate, double critic_learning_rate) : algorithm(pol, cr, envi, policy_learning_rate, critic_learning_rate){
    }
...
//    critic->to(torch::kDouble);
    ...
    std::tuple<torch::Tensor, torch::Tensor> calculate_gradient() {
    ...
    void update_policy_weights(torch::Tensor policy_loss_){
    ...
    void update_critic_weights(torch::Tensor critic_loss_){
